...
describe('PurchasedList', () => {
  ...
  it('sorts by urgency (most overdue first) when restock mode is set, with no-history items last', () => {
    const now = Date.now();
    renderWithItems(
      [
        {
          id: '1',
          name: 'PlentyOfTime',
          purchaseHistory: [now - 8 * MS_PER_DAY, now - 1 * MS_PER_DAY],
          purchaseOrder: 0,
          bought: true,
          quantity: 1,
        },
        {
          id: '2',
          name: 'NoBadge',
          purchaseHistory: [now - 5 * MS_PER_DAY],
          purchaseOrder: 1,
          bought: true,
          quantity: 1,
        },
        {
          id: '3',
          name: 'Overdue',
          purchaseHistory: [now - 20 * MS_PER_DAY, now - 15 * MS_PER_DAY],
          purchaseOrder: 2,
          bought: true,
          quantity: 1,
        },
        {
          id: '4',
          name: 'DueToday',
          purchaseHistory: [now - 14 * MS_PER_DAY, now - 7 * MS_PER_DAY],
          purchaseOrder: 3,
          bought: true,
          quantity: 1,
        },
    ...
    const items = screen.getAllByRole('checkbox');
    expect(items[0]).toHaveAccessibleName(/Overdue/);
    expect(items[1]).toHaveAccessibleName(/DueToday/);
    expect(items[2]).toHaveAccessibleName(/PlentyOfTime/);
    expect(items[3]).toHaveAccessibleName(/NoBadge/);
  });

  it('toggles to restock sort', () => {
    const now = Date.now();
    renderWithItems([
      {
        id: '1',
